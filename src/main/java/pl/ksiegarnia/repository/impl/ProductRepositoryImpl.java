...
public class ProductRepositoryImpl implements ProductRepository {
...

	{
		public Book mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			Book book = new Book.Builder().idksiazki(rs.getInt("idksiazki")).imieautora(rs.getString("imieautora"))
					.nazwiskoautora(rs.getString("nazwiskoautora")).tutyl(rs.getString("tytul"))
					.cena(rs.getFloat("cena")).ilosc(rs.getInt("ilosc")).build();
	...
	public Book getProductById(int id) {
		return jdbcTemplate.queryForObject("select * from ksiazki where idksiazki = ?", new maper(), id);
	}

	public int getIdFromProduct(String imieautora, String nazwiskoautora, String tytul, float cena, int ilosc) {
		return jdbcTemplate.queryForObject(
				"select idksiazki from ksiazki where imieautora = ? " + "AND nazwiskoautora = ?" + "AND tytul = ?"
						+ "AND cena = ?" + "AND ilosc = ? ",
				Integer.class, imieautora, nazwiskoautora, tytul, cena, ilosc);
	}
	...
	public void dodaj(Book book) {
		jdbcTemplate.update("INSERT INTO ksiazki(imieautora, nazwiskoautora, tytul, cena, ilosc) VALUES(?, ?, ?, ?, ?)",
				book.getImieautora(), book.getNazwiskoautora(), book.getTytul(), book.getCena(), book.getIlosc());
	}
	...
	public void updateBook(Book book) {
		jdbcTemplate.update(
				"UPDATE ksiazki SET cena=?, ilosc=?, imieautora=?, nazwiskoautora=?, tytul = ? WHERE idksiazki = ?",
				book.getCena(), book.getIlosc(), book.getImieautora(), book.getNazwiskoautora(), book.getTytul(),
				book.getIdksiazki());
